...
- **宿主请求头可选注入**：插件入口尝试取 AstrBot 通用头（如 UA）传给共享请求层，覆盖搜索、抓取与 `/v1/models` 连通性检查（含流式/重试）；优先级为宿主默认头 < `extra_headers` < 固定鉴权头，字段名大小写不敏感；接口缺失时静默跳过、不影响请求
...
- 共享网页抓取保留服务端 `model` / `usage`，恢复 Skill 默认诊断 JSON 契约；`--output llm` 仍排除诊断元信息
...
- 统一搜索证据规则与深度引导，明确搜索、网页读取和反向搜图的适用场景
- LLM 工具始终保留可用来源，展示开关和数量上限仅影响指令结果

### Added

- Skill 新增 `--output llm` 精简输出；默认 JSON 输出保持兼容

### Fixed

- Skill 搜索正确使用自定义提示词，Responses 配置下的网页读取使用正确响应解析器
- 网页读取保留正文中的 JSON 示例，并清理首尾终端装饰，避免原始诊断信息进入模型上下文
